    ...
    if ($_SESSION["cargo"] != "admin" || !isset($_SESSION["cargo"])) {
        ...
        if ($result_set) {
            while ($row = $result_set->fetch_assoc()) {
                ...
                $fimevento = $row['fim_prova'];
                $inicioevento = $row['inicio_prova'];
            }
        }
        ?>
        ...
        if (isset($_POST["editar2"])) {
            ...
            if ($result_set1) {
                ...
                if ($result_set1->num_rows == 0) {
                    $dataAnterior = $inicioevento; //mudar isto
                    echo "<script>alert('" . $dataAnterior . "<-data Anterior " . $antes . "<-ronda anterior')</script>";
                } else {
            ...
            if ($result_set2) {
                if ($result_set2->num_rows == 0) {
                    while ($row2 = $result_set2->fetch_assoc()) {
                        $dataSeguinte = $fimevento; //mudar isto
                    }
                } else {
                    while ($row2 = $result_set2->fetch_assoc()) {
                ...
                if ($result_set->num_rows == 1 && $rondanovo != $numero) {
                    ...
                    if ($result_set) {
                        ...
                        <script>
                            ...
                            let link = "etapasManagement.php?id=" + id_prova;
                            window.setTimeout(function () {
                                location.href = link;
                            }, 0);
                        </script>
