...
<html>
    ...
    <body>
        ...
        <div style="background-color:#1b1b1b; float: left; width:80% ">
            ...
            <br>
                <Fieldset>
                    <legend align="left"> <font face="arial" size="4" color="#ffc119"><?php echo $nomeuser ?> <?php echo $pesquisa ?> list</font>
                    </legend>
                    <?php
                $servername = "localhost";
                $username = "root";
                ...
                // SELECT (whatever rows you want) FROM (your table name)
                $sql = "SELECT animes.idAnime, animes.nomeAnime, animes.keyvisual, status.idAnime, status.id, status.status FROM animes INNER JOIN status ON animes.idAnime=status.idAnime where id='$idutilizador' AND status='$pesquisa' ORDER BY nomeAnime";
                $result = $conn->query($sql);
                $numerolinha=1;
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "$nomeuser doesn't have any anime marked as $pesquisa";
                }
                $conn->close();
            ?>
            </fieldset>
